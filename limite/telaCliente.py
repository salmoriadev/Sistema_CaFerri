...
class TelaCliente:
    ...
    def init_opcoes(self):
        ...
        botoes_opcoes = [
            [sg.Button('Incluir Cliente', key='1', font='Any 12', pad=(5, 5), expand_x=True)],
            [sg.Button('Alterar Cliente', key='2', font='Any 12', pad=(5, 5), expand_x=True)],
            [sg.Button('Listar Clientes', key='3', font='Any 12', pad=(5, 5), expand_x=True)],
            [sg.Button('Excluir Cliente', key='4', font='Any 12', pad=(5, 5), expand_x=True)],
            [sg.Button('Ver Recomendações de Café', key='5', font='Any 12', pad=(5, 5), expand_x=True)],
            [sg.Button('Retornar', key='0', font='Any 12', pad=(5, 5), expand_x=True)]
        ]

        layout = [
            [sg.Column([[sg.Text('-------- Clientes ---------', font=("Helvica", 25), pad=((0,0),(20,10)))]], justification='center')],
            [sg.Column([[sg.Text('Escolha sua opção', font=("Helvica", 15), pad=((0,0),(0,20)))]], justification='center')],
            [sg.Column([[sg.Frame('Ações', botoes_opcoes, font='Any 14', title_color='white')]], justification='center')]
        ]

        self.__window = sg.Window('Gerenciamento de Clientes', layout, element_justification='center', size=(550, 450))
    ...
    def pega_dados_cliente(self, perfil_mapa: dict, is_alteracao: bool = False) -> Optional[Dict]:
        ...
        if not is_alteracao:
            layout_formulario.append([sg.Text('ID:', size=(15, 1)), sg.Input(key='id')])
        
        layout_formulario.append([sg.Text('Nome:', size=(15, 1)), sg.Input(key='nome')])
        layout_formulario.append([sg.Text('Email:', size=(15, 1)), sg.Input(key='email')])
        
        if is_alteracao:
            layout_formulario.append([sg.Text('Nova Senha:', size=(15, 1)), sg.Input(key='senha', password_char='*')])
            layout_formulario.append([sg.Text('(Deixe em branco para manter a atual)', font='Any 8', text_color='yellow')])
        else:
            layout_formulario.append([sg.Text('Senha:', size=(15, 1)), sg.Input(key='senha', password_char='*')])

        layout_formulario.append([sg.Text('Saldo:', size=(15, 1)), sg.Input(key='saldo')])
        
        layout_formulario.append([sg.Text('Perfil:', size=(15, 1)), 
                                  sg.Combo(perfis_disponiveis, key='perfil', size=(30, 1), readonly=True)])

        layout_formulario.append([sg.Button('Confirmar', bind_return_key=True), sg.Button('Cancelar')])
        ...
        while True:
            ...
            if event == 'Confirmar':
                try:
                    ...
                    if not (email and '@' in email and '.' in email): raise ValueError("Email inválido.")
                    
                    senha = values['senha']
                    if not is_alteracao and not senha: raise ValueError("Senha obrigatória na inclusão.")
                    
                    saldo = float(values['saldo'])
                    if saldo < 0: raise ValueError("Saldo não pode ser negativo.")
                    
                    perfil = values['perfil']
                    if not perfil: raise ValueError("Selecione um perfil.")
                    ...
                    if not is_alteracao:
                        ...
                        if id_cliente < 0: raise ValueError("ID não pode ser negativo.")
                        dados["id"] = id_cliente
                        dados["senha"] = senha
                    elif senha:
                        dados["senha"] = senha
    ...
    def seleciona_cliente(self) -> Optional[int]:
        ...
        layout = [
            ...
            [sg.Text('ID do Cliente:', size=(15, 1)), sg.Input(key='id_cliente')],
    ...
    def pedir_senha(self) -> Optional[str]:
        senha = sg.popup_get_text("Digite sua senha atual para confirmar:", password_char='*', title="Segurança")
        return senha
